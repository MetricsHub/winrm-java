...
class NTLMMessage {
	...
	NTLMMessage(final byte[] message, final int expectedType) throws NtlmException {
		...
		if (messageContents.length < SIGNATURE.length) {
			throw new NtlmException("NTLM message decoding error - packet too short");
		}
		int i = 0;
		while (i < SIGNATURE.length) {
			if (messageContents[i] != SIGNATURE[i]) {
				throw new NtlmException("NTLM message expected - instead got unrecognized bytes");
			}
			i++;
		}
		...
		if (type != expectedType) {
			throw new NtlmException(String.format("NTLM type %d message expected - instead got type %d", expectedType, type));
		}
	 ...
	 */
	String getResponse() {
		return new String(java.util.Base64.getEncoder().encode(getBytes()), NTLMEngineUtils.DEFAULT_CHARSET);
	}
